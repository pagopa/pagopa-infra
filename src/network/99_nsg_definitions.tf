...
locals {
  ...
  weu_postgres_base_inbound_rules = concat([
    ...
    var.enabled_features.all_vnet_database_access ? [] : [local.nsg_rule_library.deny_from_all_vnet]
  ...
  nsg_rule_library = {
    ...
    allow_fdr_archive_to_postgres = {
      ...
      priority                = 540
      target_service          = "postgresql"
      source_subnet_name      = "pagopa-${var.env_short}-weu-fdr-archive-pgres-flexible-snet"
      source_subnet_vnet_name = "pagopa-${var.env_short}-vnet"
