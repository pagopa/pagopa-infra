...
module "apim_node_for_ecommerce_api_v1" {
  ...
  content_value = templatefile("./api/nodopagamenti_api/nodoPerEcommerce/v1/_openapi.json.tpl", {
    host    = local.api_domain
    service = module.apim_nodo_dei_pagamenti_product.product_id
  })

  xml_content = templatefile("./api/nodopagamenti_api/nodoPerEcommerce/v1/_base_policy.xml.tpl", {
...
resource "azurerm_api_management_api_operation_policy" "close_payment_api_v1" {
  ...
  xml_content = templatefile("./api/nodopagamenti_api/nodoPerEcommerce/v1/_add_v1_policy.xml.tpl", {
    is-nodo-decoupler-enabled = var.apim_nodo_decoupler_enable
  })
}
...
module "apim_node_for_ecommerce_api_v2" {
  ...
  content_format = "openapi"
  content_value = templatefile("./api/nodopagamenti_api/nodoPerEcommerce/v2/_openapi.json.tpl", {
    host = local.api_domain
  })

  xml_content = templatefile("./api/nodopagamenti_api/nodoPerEcommerce/v2/_base_policy.xml.tpl", {
    is-nodo-decoupler-enabled = var.apim_nodo_decoupler_enable
  })
}
...
resource "azurerm_api_management_api_operation_policy" "close_payment_api_v2_wisp_policy" {
  ...
  xml_content         = file("./api/nodopagamenti_api/nodoPerEcommerce/v2/wisp-closepayment.xml")
}
