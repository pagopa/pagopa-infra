resource "azurerm_resource_group" "payment_manager_rg" {
  count    = var.env_short == "p" ? 1 : 0
  name     = format("%s-payment-manager-rg", local.project)
...
resource "azurerm_monitor_scheduled_query_rules_alert" "pm_restapi_availability" {
  ...
  resource_group_name = azurerm_resource_group.payment_manager_rg[0].name
  location            = var.location
