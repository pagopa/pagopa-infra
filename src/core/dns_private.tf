...
resource "azurerm_private_dns_a_record" "platform_dns_a_private_apim" {
  ...
  records             = var.enabled_features.apim_v2 ? data.azurerm_api_management.apim_v2[0].private_ip_addresses : module.apim[0].private_ip_addresses
  tags                = var.tags
}
...
resource "azurerm_private_dns_a_record" "platform_dns_a_private_prf" {
  ...
  resource_group_name = azurerm_resource_group.rg_vnet.name
  ttl                 = var.dns_default_ttl_sec
  records             = var.enabled_features.apim_migrated ? data.azurerm_api_management.apim_migrated[0].private_ip_addresses : module.apim[0].private_ip_addresses
  tags                = var.tags
}
