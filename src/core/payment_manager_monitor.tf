...
resource "azurerm_monitor_scheduled_query_rules_alert" "pm_restapi_cd_availability" {
  ...
  action {
    action_group           = [azurerm_monitor_action_group.email.id, azurerm_monitor_action_group.slack.id, azurerm_monitor_action_group.mo_email.id, azurerm_monitor_action_group.pm_opsgenie.id]
    email_subject          = "Email Header"
    custom_webhook_payload = "{}"
  }
