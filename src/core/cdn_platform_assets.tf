 ...
 **/
resource "azurerm_resource_group" "platform_assets_rg" {
  count    = var.env_short == "p" ? 1 : 0
  name     = format("%s-platform-assets-rg", local.project)
 ...
 * CDN
 */
module "platform_assets_cdn" {
  source                = "git::https://github.com/pagopa/azurerm.git//cdn?ref=v3.2.0"

  count                 = var.env_short == "p" ? 1 : 0
  name                  = "platform-assets"
  prefix                = local.project
  resource_group_name   = azurerm_resource_group.platform_assets_rg[0].name
  location              = var.location
  hostname              = "assets.platform.pagopa.it"
