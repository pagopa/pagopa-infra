...
locals {
  ...
  pagopa-portal-hostname          = "welfare.${azurerm_dns_zone.public.name}"
  selfcare-issuer                = "https://${var.env_short != "prod" ? "${var.env}." : ""}selfcare.pagopa.it"
}

resource "azurerm_key_vault_certificate" "pagopa_jwt_signing_cert" {
  name         = "${local.project}-${var.external_domain}-jwt-signing-cert"
  key_vault_id = module.key_vault.id
...
resource "azurerm_api_management_certificate" "pagopa_token_exchange_cert_jwt" {
  name                = "${local.project}-${var.external_domain}-token-exchange-jwt"
  api_management_name = module.apim.name
