...
module "apim_debt_positions_product" {
  ...
  product_id   = "product-gpd-auth"
  display_name = "GPD pagoPA Auth"
  description  = "Prodotto Gestione Posizione Debitorie Auth"
  ...
  policy_xml = file("./api_product/gpd/auth/_base_policy.xml")
}

locals {
  apim_debt_positions_service_api = {
    display_name          = "GPD pagoPA - Debt Positions service API Auth"
    description           = "GPD pagoPA - Debt Positions service API Auth"
    path                  = "gpd-auth/api"
    subscription_required = true
    service_url           = null
  }
...
resource "azurerm_api_management_api_version_set" "api_debt_positions_api" {
  ...
  name                = format("%s-debt-positions-service-api-auth", var.env_short)
  resource_group_name = azurerm_resource_group.rg_api.name
  api_management_name = module.apim.name
...
module "apim_api_debt_positions_api_v1" {
  ...
  name                  = format("%s-debt-positions-service-api-auth", local.project)
  api_management_name   = module.apim.name
  resource_group_name   = azurerm_resource_group.rg_api.name
  ...
  xml_content = templatefile("./api/gpd_api/auth/v1/_base_policy.xml", {
