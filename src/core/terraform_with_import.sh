...
function extract_resources() {
  ...
  echo "./terraform_with_import.sh $action $ENV $TARGETS"
}

function help_usage() {
  echo "terraform_with_import.sh Version ${vers}"
  echo
...
# Funzione per parsare i log ed eseguire l'import automatico
function auto_import_resources() {
  local log_file="$1"
  local env_vars="$2"

  # Utilizzo perl per parsare l'errore multiriga tipico del provider AzureRM e compatibilità tra macOS/Linux.
  # riga 1: A resource with the ID "..." already exists
  # riga 2: with azurerm_resource_group.nome,
  local imports
  imports=$(perl -nle '
    if (/A resource with the ID "([^"]+)" already exists/) { $id=$1; }
    elsif (/with\s+([^,\s]+),/ && $id) { print "$1|$id"; $id=""; }
  ' "$log_file")

  if [ -z "$imports" ]; then
    return 1 # Nessun errore di 'already exists' parsato con successo
  fi

  echo ""
  echo "${yellow}${bold}>>> Trovate risorse da importare nello State. Inizio import automatico...${normal}"

  local success=0
  for item in $imports; do
    local addr="${item%|*}"
    local id="${item#*|}"
    echo "${blue}[+] Importazione in corso:${normal} Target: $addr | ID: $id"
    terraform import $env_vars "$addr" "$id"
    if [ $? -eq 0 ]; then
        echo "    ${green}Import completato con successo!${normal}"
        success=1
    else
        echo "    ${red}Errore durante l'import.${normal}"
    fi
  done

  # Ritorna 0 se almeno uno ha funzionato (così scatta il retry), altrimenti 1
  return $(( 1 - success ))
}
...
function other_actions() {
  if [ -n "$env" ] && [ -n "$action" ]; then
    ...
    # Se apply in ambiente prod con regole audit definite
    if [ "$action" == "apply" ] && [[ "$env" == *"prod" ]] && [ -f "$root_folder/.terraform-audit" ]; then
      ...
      # Ciclo retry automatico con parsing log fallimento per import automatico (max 3 tentativi)
      local attempt=1
      local max_attempts=3
      while [ $attempt -le $max_attempts ]; do
        plan_output_file=$(mktemp)

        # Esegue il terraform e usa 'tee' per scrivere a terminale e dentro il file temp per poter essere analizzato
        ...
        if [ $plan_exitcode -eq 1 ] && grep -q "already exists" "$plan_output_file"; then
          ...
          if auto_import_resources "$plan_output_file" "-var-file=./env/$env/terraform.tfvars"; then
            ...
            echo "${blue}>>> Rieseguo il comando Terraform Plan per riconciliare lo stato...${normal}"
            continue
          fi
        fi
        rm -f "$plan_output_file"
        break # Esci dal ciclo se va a buon fine o se è un altro tipo di errore
      done
      ...
      while [ $attempt -le $max_attempts ]; do
        ...
        if [ $cmd_exitcode -ne 0 ] && grep -q "already exists" "$cmd_output_file"; then
           echo "${yellow}Rilevato errore di risorsa esistente. Tento l'import automatico (Tentativo $attempt di $max_attempts)...${normal}"
           if auto_import_resources "$cmd_output_file" "-var-file=./env/$env/terraform.tfvars"; then
             attempt=$((attempt + 1))
             rm -f "$cmd_output_file"
             echo "${blue}>>> Rieseguo il comando Terraform...${normal}"
             continue
           fi
        fi
      ...
      # Propaga lo status di uscita all'utente per fallimenti genuini
