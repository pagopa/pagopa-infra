...
locals {
  apim_tkm_consent_manager_api = {
    # params for all api versions
    display_name          = "Payment Manager - tkm-ms-consent-manager"
...
module "apim_tkm_consent_manager_api_v1" {
  ...
  content_format = "openapi"
  content_value = templatefile("./api/payment_manager_api/tkm-ms-consent-manager/v1/_openapi.json.tpl", {
    host = azurerm_api_management_custom_domain.api_custom_domain.proxy[0].host_name
  })
...
module "apim_tkm_card_manager_api_v1" {
  ...
  content_format = "openapi"
  content_value = templatefile("./api/payment_manager_api/tkm-ms-card-manager/v1/_openapi.json.tpl", {
    host = azurerm_api_management_custom_domain.api_custom_domain.proxy[0].host_name
  })

  xml_content = file("./api/payment_manager_api/tkm-ms-card-manager/v1/_base_policy.xml.tpl")
}
...
module "apim_tkm_acquirer_manager_api_v1" {
  ...
  content_format = "openapi"
  content_value = templatefile("./api/payment_manager_api/tkm-ms-acquirer-manager/v1/_openapi.json.tpl", {
    host = azurerm_api_management_custom_domain.api_custom_domain.proxy[0].host_name
  })

  xml_content = file("./api/payment_manager_api/tkm-ms-acquirer-manager/v1/_base_policy.xml.tpl")
}
