...
module "pagopa_proxy_redis_ha" {
  source                = "git::https://github.com/pagopa/terraform-azurerm-v3.git//redis_cache?ref=v7.69.1"
  count                 = var.redis_ha_enabled ? 1 : 0
  name                  = format("%s-pagopa-proxy-redis-ha", local.parent_project)
  resource_group_name   = azurerm_resource_group.pagopa_proxy_rg.name
  location              = azurerm_resource_group.pagopa_proxy_rg.location
  capacity              = var.pagopa_proxy_redis_ha_capacity
  enable_non_ssl_port   = false
  family                = var.pagopa_proxy_redis_ha_family
  sku_name              = var.pagopa_proxy_redis_ha_sku_name
  enable_authentication = true
