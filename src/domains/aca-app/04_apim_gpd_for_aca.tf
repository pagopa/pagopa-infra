...
module "apim_aca_upload_api_v1" {
  ...
  protocols    = ["https"]
  service_url  = local.apim_aca_upload_api.service_url

  content_format = "openapi"
  content_value = templatefile("./../gps-app/api/gpd-upload-service/v1/_openapi.json.tpl", {
    host = local.apim_hostname
  })

  xml_content = templatefile("./../gps-app/api/gpd-upload-service/v1/_base_policy.xml", {
    service_type_value = "ACA"
  })
}
