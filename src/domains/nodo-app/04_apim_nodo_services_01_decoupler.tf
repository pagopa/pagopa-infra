...
resource "null_resource" "decoupler_configuration_from_json_2_xml" {
  ...
  triggers = {
    "changes-in-config-decoupler" : sha1(file("./api_product/legacy/nodo_pagamenti_api/decoupler/cfg/${var.env}/decoupler_configuration.json"))
  }
  provisioner "local-exec" {
    command = "sh ./api_product/legacy/nodo_pagamenti_api/decoupler/cfg/decoupler_configurator.sh ${var.env}"
  }
}
...
resource "terraform_data" "sha256_decoupler_configuration" {
  input = sha256(file("./api_product/legacy/nodo_pagamenti_api/decoupler/cfg/${var.env}/decoupler-configuration.xml"))
}
resource "azapi_resource" "decoupler_configuration" {
  ...
  body = jsonencode({
    properties = {
      ...
      format      = "rawxml"
      value       = file("./api_product/legacy/nodo_pagamenti_api/decoupler/cfg/${var.env}/decoupler-configuration.xml")
    }
  })
...
resource "terraform_data" "sha256_decoupler_algorithm" {
  input = sha256(file("./api_product/legacy/nodo_pagamenti_api/decoupler/decoupler-algorithm.xml"))
}
resource "azapi_resource" "decoupler_algorithm" {
  ...
  body = jsonencode({
    properties = {
      ...
      format      = "rawxml"
      value       = file("./api_product/legacy/nodo_pagamenti_api/decoupler/decoupler-algorithm.xml")
    }
  })
...
resource "terraform_data" "sha256_decoupler_activate_inbound" {
  input = sha256(file("./api_product/legacy/nodo_pagamenti_api/decoupler/decoupler-activate-inbound.xml"))
}
resource "azapi_resource" "decoupler_activate_inbound" {
  ...
  body = jsonencode({
    properties = {
      ...
      format      = "rawxml"
      value       = file("./api_product/legacy/nodo_pagamenti_api/decoupler/decoupler-activate-inbound.xml")
    }
  })
...
resource "terraform_data" "sha256_decoupler_activate_outbound" {
  input = sha256(file("./api_product/legacy/nodo_pagamenti_api/decoupler/decoupler-activate-outbound.xml"))
}
resource "azapi_resource" "decoupler_activate_outbound" {
  ...
  body = jsonencode({
    properties = {
      ...
      format      = "rawxml"
      value       = file("./api_product/legacy/nodo_pagamenti_api/decoupler/decoupler-activate-outbound.xml")
    }
  })
...
locals {
  on_error_soap_file = templatefile("./api_product/legacy/nodo_pagamenti_api/on_error_soap_req.xml", {
    ndphost_variable = file("./api_product/legacy/nodo_pagamenti_api/env/${var.env}/ndphost_header.xml")
  })
}

resource "terraform_data" "sha256_on_erro_soap_handler" {
  input = sha256(file("./api_product/legacy/nodo_pagamenti_api/on_error_soap_req.xml"))
}
...
resource "terraform_data" "sha256_nodoinviarpt_wisp_nodoinviacarrellorpt_outbound_policy" {
  input = sha256(file("./api/legacy/nodopagamenti_api/nodoPerPa/v1/wisp_nodoInviaRPT_nodoInviaCarrelloRPT_outbound_policy.xml"))
}
resource "azapi_resource" "wisp_nodoinviarpt_nodoinviacarrellorpt_outbound_policy" {
  ...
  body = jsonencode({
    properties = {
      ...
      format      = "rawxml"
      value       = file("./api/legacy/nodopagamenti_api/nodoPerPa/v1/wisp_nodoInviaRPT_nodoInviaCarrelloRPT_outbound_policy.xml")
    }
  })
