...
resource "azurerm_api_management_api_operation_policy" "post_payment_methods_v3" {
  api_name            = "${local.project}-checkout-api-v3"
  resource_group_name = local.pagopa_apim_rg
  api_management_name = local.pagopa_apim_name
  operation_id        = "getAllPaymentMethodsAuth"

  xml_content = file("./api/ecommerce-checkout/v3/_post_payment_methods_policy.xml.tpl")
}
