...
data "azurerm_key_vault_secret" "pagopa_ecommerce_helpdesk_commands_service_primary_api_for_checkout_key" {
  name         = "${local.project}-helpdesk-commands-service-primary-api-key"
  key_vault_id = data.azurerm_key_vault.kv.id
}

data "azurerm_key_vault_secret" "pagopa_ecommerce_helpdesk_commands_service_secondary_api_for_checkout_key" {
  name         = "${local.project}-helpdesk-commands-service-secondary-api-key"
  key_vault_id = data.azurerm_key_vault.kv.id
}

resource "azurerm_api_management_named_value" "pagopa_ecommerce_helpdesk_commands_service_api_key_value_for_checkout" {
  name                = "${local.project}-helpdesk-commands-service-api-key-for-checkout-value"
  api_management_name = local.pagopa_apim_name
  resource_group_name = local.pagopa_apim_rg
  display_name        = "ecommerce-helpdesk-commands-service-api-key-for-checkout-value"
  value               = var.ecommerce_helpdesk_commands_service_api_key_use_primary ? data.azurerm_key_vault_secret.pagopa_ecommerce_helpdesk_commands_service_primary_api_for_checkout_key.value : data.azurerm_key_vault_secret.pagopa_ecommerce_helpdesk_commands_service_secondary_api_for_checkout_key.value
  secret              = true
}
