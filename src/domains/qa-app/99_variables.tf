...
variable "qa_hub_plan_idh_tier" {
  type        = string
  description = "The IDH resource tier for qa-hub"
}

variable "qa_hub_autoscale_settings" {
  type = object({
  ...
  description = "Autoscale settings for qa-hub"
}

variable "qa_hub_always_on" {
  type        = bool
