  ...
  global_delivery_rule = {
    ...
    modify_response_header_action = [{
      ...
      # Content-Security-Policy
      {
        ...
        value  = format("default-src 'self'; connect-src 'self' https://api.%s.%s https://api.%s.%s *wisp2.pagopa.gov.it *.nexigroup.com;", var.dns_zone_platform, var.external_domain)
      },
      {
        action = "Append"
        name   = "Content-Security-Policy"
