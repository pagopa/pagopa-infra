...
# Defender Storage Account
resource "azurerm_security_center_storage_defender" "fdr_storage_defender" {
  count = var.fdr_storage_account.storage_defender_enabled ? 1 : 0

  # storage_account_id                          = module.fdr_re_sa.id
  storage_account_id                          = module.fdr_conversion_sa
  override_subscription_settings_enabled      = var.fdr_storage_account.storage_defender_override_subscription_settings_enabled
  sensitive_data_discovery_enabled            = var.fdr_storage_account.storage_defender_sensitive_data_discovery_enabled
  malware_scanning_on_upload_enabled          = var.fdr_storage_account.storage_defender_malware_scanning_on_upload_enabled
  malware_scanning_on_upload_cap_gb_per_month = var.fdr_storage_account.storage_defender_malware_scanning_on_upload_cap_gb_per_month
...
resource "azurerm_storage_management_policy" "fdr1_re_payload_blob_file_management_policy" {
  ...
  rule {
    ...
    actions {
      ...
      base_blob {
        delete_after_days_since_modification_greater_than = var.fdr_storage_account.blob_file_retention_days
      }
      snapshot {
        delete_after_days_since_creation_greater_than = 1
      }
      version {
        delete_after_days_since_creation = var.fdr_storage_account.blob_file_retention_days
      }
    }
  }
