...
resource "azurerm_monitor_scheduled_query_rules_alert" "alert_fdr_internal_availability" {
  ...
  description    = "Availability for /fdr-internal/service is less than or equal to 99% - [DASHBOARD LINK]" # todo add link
  enabled        = true
  query = (<<-QUERY
      let threshold = 0.99;
      AzureDiagnostics
...
resource "azurerm_monitor_scheduled_query_rules_alert" "alert_fdr_psp_availability" {
  ...
  data_source_id = data.azurerm_api_management.apim.id
  description    = "Availability for /fdr-psp/service is less than or equal to 99% - [DASHBOARD LINK]" # todo add link
  enabled        = true
  query = (<<-QUERY
      let threshold = 0.99;
      AzureDiagnostics
...
resource "azurerm_monitor_scheduled_query_rules_alert" "alert_fdr_org_availability" {
  ...
  data_source_id = data.azurerm_api_management.apim.id
  description    = "Availability for /fdr-org/service is less than or equal to 99% - [DASHBOARD LINK]" # todo add link
  enabled        = true
  query = (<<-QUERY
      let threshold = 0.99;
      AzureDiagnostics
