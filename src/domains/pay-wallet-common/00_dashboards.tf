...
resource "azurerm_portal_dashboard" "pay_wallet_cosmosdb_dashboard" {
  count               = var.env_short == "p" ? 1 : 0
  name                = "pay-wallet-cosmosdb-dashboard"
  resource_group_name = var.monitor_italy_resource_group_name
  location            = var.location
  tags = {
    source = "terraform"
  ...
  dashboard_properties = jsonencode(
    merge(
      jsondecode(file("./dashboards/pay-wallet-cosmosdb-dashboard.json")).properties,
      {
        "lenses" = {
          for lens_index, lens in jsondecode(file("./dashboards/pay-wallet-cosmosdb-dashboard.json")).properties.lenses :
          tostring(lens_index) => merge(lens, {
            "parts" = {
              for part_index, part in lens.parts :
              tostring(part_index) => part
