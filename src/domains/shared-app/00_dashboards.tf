resource "azurerm_portal_dashboard" "node_forwarder_dashboard" {
  ...
  dashboard_properties = jsonencode(
    merge(
      jsondecode(file("./dashboards/node-forwarder-dashboard.json")).properties,
      {
        "lenses" = {
          for lens_index, lens in jsondecode(file("./dashboards/node-forwarder-dashboard.json")).properties.lenses :
