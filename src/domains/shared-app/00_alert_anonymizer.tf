resource "azurerm_monitor_scheduled_query_rules_alert" "opex_pagopa-anonymizer-rest-responsetime-upd" {
  ...
  name                = "pagopa-${var.env_short}-alert_pagopa-gpd-responsetime @ _anonymize"
...
let threshold = 1500;
AzureDiagnostics
| where url_s matches regex "/anonymize"
| summarize
    watermark=threshold,
...
resource "azurerm_monitor_scheduled_query_rules_alert" "opex_pagopa-anonymizer-rest-availability-upd" {
  ...
  resource_group_name = "dashboards"
  name                = "pagopa-${var.env_short}-alert_pagopa-gpd-availability@ _anonymize"
  location            = var.location
  ...
  data_source_id = data.azurerm_api_management.apim.id
  description    = "Availability for /anonymize is less than or equal to 99% - https://portal.azure.com/#@pagopait.onmicrosoft.com/dashboard/arm/subscriptions/b9fc9419-6097-45fe-9f74-ba0641c91912/resourceGroups/dashboards/providers/Microsoft.Portal/dashboards/pagopa-p-opex_pagopa-anonymizer"
  enabled        = true
  query = (<<-QUERY
let threshold = 0.99;
AzureDiagnostics
