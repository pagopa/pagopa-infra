...
resource "azurerm_monitor_scheduled_query_rules_alert" "opex_pagopa-afm-calculator-v2-availability-feesbypsp" {
  ...
  data_source_id = data.azurerm_api_management.apim.id
  description    = "Availability for V2 version of /psps/[^/]+/fees is below the traffic-adjusted threshold in two 5m buckets within 10m - ${local.afm-calculator-dash.calculator-v2}"
  enabled        = true
  query = (<<-QUERY
let lowTrafficThreshold = 70;
let highTrafficThreshold = 95;
let trafficMin = 100;
let trafficLinear = 500;
let thresholdDelta = trafficLinear - trafficMin;
let availabilityDelta = highTrafficThreshold - lowTrafficThreshold;
AzureDiagnostics
| where url_s matches regex "/v2/psps/[^/]+/fees"
