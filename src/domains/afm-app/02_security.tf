...
// apikey test for AFM Marketplace
resource "azurerm_api_management_subscription" "test_afm_marketplace_subkey" {
  count               = var.env_short != "p" ? 1 : 0
  api_management_name = local.pagopa_apim_name
  resource_group_name = local.pagopa_apim_rg

  product_id    = module.apim_afm_marketplace_product.id
  display_name  = "Subscription to AFM Marketplace for integration test"
  allow_tracing = false
  state         = "active"
}

resource "azurerm_key_vault_secret" "test_afm_marketplace_kv" {
  count        = var.env_short != "p" ? 1 : 0
  depends_on   = [azurerm_api_management_subscription.test_afm_marketplace_subkey[0]]
  name         = "integration-test-afm-marketplace-subkey"
  value        = azurerm_api_management_subscription.test_afm_marketplace_subkey[0].primary_key
  content_type = "text/plain"
