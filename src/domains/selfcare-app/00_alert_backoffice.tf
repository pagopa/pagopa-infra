...
resource "azurerm_monitor_scheduled_query_rules_alert" "alert_pagopa-backoffice-responsetime" {
  for_each = var.env_short == "p" ? { for c in local.selfcare_services : c.base_path => c } : {}
  ...
  action {
    action_group           = [data.azurerm_monitor_action_group.email.id, data.azurerm_monitor_action_group.slack_pagamenti.id]
...
resource "azurerm_monitor_scheduled_query_rules_alert" "alert-pagopa-backoffice-availability" {
  ...
  action {
    ...
    ] : [data.azurerm_monitor_action_group.email.id, data.azurerm_monitor_action_group.slack.id, data.azurerm_monitor_action_group.slack_pagamenti.id]
