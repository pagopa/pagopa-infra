...
module "apim_api_apiconfig_cache_node_nexi_api_prf_v1" {
  ...
  path        = "${local.apiconfig_cache_locals.path}/oprf"
  protocols   = ["https"]
  service_url = local.apiconfig_cache_locals.service_url
  ...
  xml_content = templatefile("./api/apiconfig-cache/node/_base_policy.xml", {
    hostname = local.apiconfig_cache_locals.hostname
    hostname = format("%s/%s", local.apiconfig_cache_locals.hostname, "${local.apiconfig_cache_locals.path}prf/o")
  })
}
