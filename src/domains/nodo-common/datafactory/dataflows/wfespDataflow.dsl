...
source(output(
     ...
     format: 'query') ~> redirectMyBankRead
...
carrelloRptRead alterRow(deleteIf(toDate(toString(inserted_timestamp,'dd-MM-yyyy','it-IT'),'dd-MM-yyyy','it-IT')<currentDate())) ~> markRecordCarrelloRptToDelete
redirectMyBankRead alterRow(deleteIf(toDate(toString(inserted_timestamp,'dd-MM-yyyy','it-IT'),'dd-MM-yyyy','it-IT')<currentDate())) ~> markRecordRedirectMyBankToDelete
markRecordRptToDelete sink(allowSchemaDrift: true,
     validateSchema: false,
     input(
     ...
     saveOrder: 1) ~> executeRptDeleteDB
markRecordCarrelloRptToDelete sink(allowSchemaDrift: true,
     validateSchema: false,
     input(
          obj_id as decimal(0,0),
          id_carrello as string,
     ...
     skipDuplicateMapInputs: true,
     skipDuplicateMapOutputs: true,
     saveOrder: 2) ~> executeCarrelloRptDeleteDB
markRecordRedirectMyBankToDelete sink(allowSchemaDrift: true,
     validateSchema: false,
     input(
          obj_id as decimal(0,0),
          id_dominio as string,
     ...
     skipDuplicateMapInputs: true,
     skipDuplicateMapOutputs: true,
     saveOrder: 3) ~> executeRedirectMyBankDeleteDB
