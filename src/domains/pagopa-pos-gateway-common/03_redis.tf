...
resource "azurerm_key_vault_secret" "redis_pagopa_pos_gateway_access_key" {
  name         = "redis-${local.domain}-access-key"
  value        = module.redis.primary_access_key
  key_vault_id = data.azurerm_key_vault.domain_kv.id
}

resource "azurerm_key_vault_secret" "redis_pagopa_pos_gateway_hostname" {
  name         = "redis-${local.domain}-hostname"
  value        = module.redis.hostname
  key_vault_id = data.azurerm_key_vault.domain_kv.id
}

resource "azurerm_key_vault_secret" "redis_pagopa_pos_gateway_connection_string" {
  name         = "redis-${local.domain}-hostname"
