...
module "postgres_flexible_snet_replica" {
  ...
  address_prefixes                              = ["10.2.141.0/24"]
...
module "postgresql_gpd_replica_db" {
  ...
  name                = "${var.prefix}-${var.env_short}-neu-${var.domain}-pgflex"
...
resource "azurerm_postgresql_flexible_server_virtual_endpoint" "virtual_endpoint" {
  ...
  replica_server_id = module.postgresql_gpd_replica_db[0].id #fixme move to itn replica
  type              = "ReadWrite"
}
