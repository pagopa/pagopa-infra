...
###################################################
locals {
  apim_ecommerce_transaction_results_update_service_api = {
    display_name          = "ecommerce pagoPA - transaction results update service API"
    description           = "API to support Nodo.sendPaymentResult"
    path                  = "ecommerce/transaction-results-update-service"
    subscription_required = true
    service_url           = null
  }
}

# Transaction results update service APIs
resource "azurerm_api_management_api_version_set" "ecommerce_transaction_results_update_service_api" {
  name                = format("%s-transaction-results-update-service-api", local.project)
  resource_group_name = local.pagopa_apim_rg
  api_management_name = local.pagopa_apim_name
  display_name        = local.apim_ecommerce_transaction_results_update_service_api.display_name
  versioning_scheme   = "Segment"
}

module "apim_ecommerce_transaction_results_update_service_api_v1" {
  source = "git::https://github.com/pagopa/azurerm.git//api_management_api?ref=v2.18.3"
  ...
  resource_group_name   = local.pagopa_apim_rg
  product_ids           = [module.apim_ecommerce_product.product_id]
  subscription_required = local.apim_ecommerce_transaction_results_update_service_api.subscription_required
  version_set_id        = azurerm_api_management_api_version_set.ecommerce_transaction_results_update_service_api.id
  api_version           = "v1"

  description  = local.apim_ecommerce_transaction_results_update_service_api.description
  display_name = local.apim_ecommerce_transaction_results_update_service_api.display_name
  path         = local.apim_ecommerce_transaction_results_update_service_api.path
  protocols    = ["https"]
  service_url  = local.apim_ecommerce_transaction_results_update_service_api.service_url
