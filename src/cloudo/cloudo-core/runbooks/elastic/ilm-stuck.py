...
def main():
  ...
  response = requests.post(f"{secrets.get(ELASTIC_BASE_URL_SECRET_NAME)}/_health_report/ilm", headers={"Authorization": f"ApiKey {secrets.get(ELASTIC_API_KEY_SECRET_NAME)}"})
  if response.status_code != 200:
    print(f"Error during cluster health report: {response.status_code} - {response.text}")
    exit(1)
  else:
    print(f"Cluster health report: {response.text}")
    response_j = json.load(response.text)
    ...
    if ilm_status == "yellow":
      ...
      for diagnosis in response_j.get("indicators").get("ilm").get("diagnosis"):
        ...
        if "elasticsearch:health:ilm:diagnosis:stagnating_action" in diagnosis_id:
          print(f"Found stagnating action: {diagnosis_id}")
          affected_idxs = diagnosis.get('affected_resources').get('indices')
          print(f"affected ix: {affected_idxs}")
          ...
          for idx in affected_idxs:
            ...
            if set_replica_response.status_code != 200:
              print(f"[ERROR] set replica to 0 failed for index {idx}")
