...
locals {
  ...
  paymentspull_space = replace(trimsuffix(trimprefix(templatefile("${path.module}/log-template/space.json", {
  ...
  pagopapaymentspull_ingest_pipeline = replace(trimsuffix(trimprefix(file("${path.module}/log-template/ingest-pipeline.json"), "\""), "\""), "'", "'\\''")
  pagopapaymentspull_ilm_policy = replace(trimsuffix(trimprefix(templatefile("${path.module}/log-template/ilm-policy.json", {
    name        = local.pagopapaymentspull_key,
    managed     = false,
    policy_name = local.default_snapshot_policy_key
  }), "\""), "\""), "'", "'\\''")
  pagopapaymentspull_component_template_package = replace(trimsuffix(trimprefix(templatefile("${path.module}/log-template/component@package.json", {
    name = local.pagopapaymentspull_key
  }), "\""), "\""), "'", "'\\''")
  pagopapaymentspull_component_template_custom = replace(trimsuffix(trimprefix(templatefile("${path.module}/log-template/component@custom.json", {
    name = local.pagopapaymentspull_key
  }), "\""), "\""), "'", "'\\''")
  pagopapaymentspull_index_template = replace(trimsuffix(trimprefix(templatefile("${path.module}/log-template/index-template.json", {
    name                       = local.pagopapaymentspull_key
    component_template_package = "${local.pagopapaymentspull_key}@package"
    component_template_custom  = "${local.pagopapaymentspull_key}@custom"
  }), "\""), "\""), "'", "'\\''")

  pagopapaymentspull_data_view = replace(trimsuffix(trimprefix(templatefile("${path.module}/log-template/data-view.json", {
    name = local.pagopapaymentspull_key
  }), "\""), "\""), "'", "'\\''")
}
...
resource "null_resource" "pagopapaymentspull_component_template_package" {
  ...
  provisioner "local-exec" {
    command     = <<EOT
      curl -k -X PUT "${local.elastic_url}/_component_template/${local.pagopapaymentspull_key}@package" \
      -H 'kbn-xsrf: true' \
      -H 'Content-Type: application/json' \
      -d '${local.pagopapaymentspull_component_template_package}'
    EOT
    interpreter = ["/bin/bash", "-c"]
  }
}
...
resource "null_resource" "pagopapaymentspull_component_template_custom" {
  depends_on = [null_resource.pagopapaymentspull_component_template_package]
