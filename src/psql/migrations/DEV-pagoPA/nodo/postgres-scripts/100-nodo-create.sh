...
init () {
    ...
    echo "POSTGRES_DB_HOST >>> ($POSTGRES_DB_HOST) $PG_USER"

    PGPASSWORD="$POSTGRES_PASSWORD" psql -h "$POSTGRES_DB_HOST" -p "$POSTGRES_DB_PORT" -v ON_ERROR_STOP=1 --username "$PG_USER" --dbname "$PG_DB"
