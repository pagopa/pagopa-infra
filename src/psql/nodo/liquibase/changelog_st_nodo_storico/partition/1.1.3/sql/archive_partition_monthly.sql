-- PROCEDURE: partition.archive_partition_monthly()

-- DROP PROCEDURE IF EXISTS partition.archive_partition_monthly();

CREATE OR REPLACE PROCEDURE partition.archive_partition_monthly(
	)
LANGUAGE 'plpgsql'
AS $BODY$
	DECLARE
    ...
    tab_cursor CURSOR FOR
      ...
      FROM dblink(
          'conn_db_link',
          'SELECT NOME_TABELLA, NOME_SCHEMA, NOME_PARTITION
           FROM partition.storico_part
           WHERE STATO=''N''
           AND nome_partition LIKE ''%_P______''
           ORDER BY 3'
      ) AS t(campo1 TEXT, campo2 TEXT, campo3 TEXT);
...
BEGIN
    ...
    INSERT INTO PARTITION.pg_log
        VALUES(iidtrace, sutente, 'archive_partition_monthly', clock_timestamp(), clock_timestamp(),
               interval '0', 'OK', 'INIZIO', tlabelstep);

    -- Verifico connessione
    BEGIN
        SELECT elem
        INTO tconndblink
        FROM (SELECT unnest(dblink_get_connections()::text[]) AS elem) x
    ...
    IF tconndblink IS NULL OR tconndblink != 'conn_db_link' THEN
...
------------------------------------------------------------------------------------------------------
      ...
      END LOOP;
      CLOSE campi_cursor;
	  /* call online.PR_LOG(ptProcesso => CAST('archive_partition_monthly' AS TEXT),
		ptDataInizio => now(),
...
END;
$BODY$;
ALTER PROCEDURE partition.archive_partition_monthly()
    OWNER TO azureuser;

GRANT EXECUTE ON PROCEDURE partition.archive_partition_monthly() TO PUBLIC;

GRANT EXECUTE ON PROCEDURE partition.archive_partition_monthly() TO azureuser;

GRANT EXECUTE ON PROCEDURE partition.archive_partition_monthly() TO partition;
