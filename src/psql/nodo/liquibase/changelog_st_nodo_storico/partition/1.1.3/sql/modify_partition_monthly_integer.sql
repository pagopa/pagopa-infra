-- PROCEDURE: partition.modify_partition_monthly(integer)
...
-- COMMENTO
RAISE NOTICE 'Procedura eseguita, log inserito';
...
IF loop_counter=0 THEN
  L_PART_LIST  = DATE_TRUNC('MONTH', CURRENT_DATE - interval '4 years')::DATE;
  L_PART_LISTB = DATE_TRUNC('MONTH', CURRENT_DATE)::DATE;
  L_PARTNAME  := TAB_RECORD||'_PMINVALUE';
ELSE
  L_PART_LIST = DATE_TRUNC('MONTH', CURRENT_DATE + ((loop_counter - 1) || ' month')::INTERVAL)::DATE;
  L_PART_LISTB = DATE_TRUNC('MONTH', CURRENT_DATE + (loop_counter || ' month')::INTERVAL)::DATE;
...
END;
$BODY$;
ALTER PROCEDURE partition.modify_partition_monthly(integer)
    OWNER TO partition;

GRANT EXECUTE ON PROCEDURE partition.modify_partition_monthly(integer) TO PUBLIC;

GRANT EXECUTE ON PROCEDURE partition.modify_partition_monthly(integer) TO partition;
