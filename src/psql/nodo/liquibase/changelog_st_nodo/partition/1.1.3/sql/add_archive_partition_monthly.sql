-- PROCEDURE: partition.add_archive_partition_monthly()

CREATE OR REPLACE PROCEDURE partition.add_archive_partition_monthly(
	)
LANGUAGE 'plpgsql'
AS $BODY$
DECLARE
...
sUtente         TEXT := USER;
tLabelStep TEXT;
tProcedureName TEXT:= 'add_archive_partition_monthly';
...
tab_cursor CURSOR FOR
	...
	JOIN partition.STORICO_CONFIG sc
	    ON lower(sc.NOME_TABELLA) = lower(parent.relname)
	   AND lower(sc.NOME_SCHEMA)  = lower(nmsp_parent.nspname)
	WHERE
	    -- SOLO partizioni mensili
	    child.relname ~ '_P[0-9]{6}$'
        AND child.relname NOT LIKE '%MINVALUE'
	    AND sc.STATO = 'Y'
	    AND sc.tipo  = 'mese'
	    AND
	    (
	        ...
	        to_char(
	            ...
	            - (sc.mesi_retention || ' month')::interval,
	            'YYYYMM'
	        )::int
	    );
...
BEGIN
	...
	OPEN tab_cursor;
   	LOOP
        ...
        RAISE NOTICE 'Schema=% Tabella=% Partizione=%',
            tNomeSchema, tNomeTabella, tNomePartizione;
...
END;
$BODY$;
ALTER PROCEDURE partition.add_archive_partition_monthly()
    OWNER TO partition;

GRANT EXECUTE ON PROCEDURE partition.add_archive_partition_monthly() TO PUBLIC;

GRANT EXECUTE ON PROCEDURE partition.add_archive_partition_monthly() TO partition;
