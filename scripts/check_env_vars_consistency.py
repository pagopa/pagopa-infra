    ...
    while i < len(lines):
        ...
        if match:
            ...
            while i < len(lines) and (brace_depth > 0 or bracket_depth > 0 or
                                      (not in_string and not stripped.strip().endswith(',') and not stripped.strip() == '}')):
                line = lines[i]
                stripped = line.strip()
                if not stripped or stripped.startswith('#'):
                    i += 1
...
def check_module(module: Path) -> List[str]:
    ...
    try:
        env_subdirs = sorted(d for d in env_dir.iterdir() if d.is_dir() and not d.name.startswith('.'))
    except OSError:
        return modifications
...
def main() -> None:
    ...
    if all_modifications:
        print("[INFO] Terraform env variables consistency check - modifications made:\n")
    ...
    print(f"Env variables consistency check completed for {len(modules)} module(s).")
    sys.exit(0)
